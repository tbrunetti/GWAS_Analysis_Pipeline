...
class Pipeline(BasePipeline):
	...
	def run_pipeline(self, pipeline_args, pipeline_config):
		...
		# as long as --reanalyze is not set, the pipeline will run through this		
		...
		def run_hwe(groupToProcess, shared_obj):
			print "running HWE step"
			general_plink.run(
				Parameter('--bfile', str(groupToProcess)),
				Parameter('--hardy'),
				Parameter('--hwe', pipeline_args['hweThresh']),
				Parameter('midp'),
				Parameter('--make-bed'),
				Parameter('--out', str(groupToProcess) + '_hweFiltered')
				)
				
			total_snps_analyzed_hwe = subprocess.check_output(['wc', '-l',  groupToProcess + '.bim'])
			total_snps_passing_hwe = subprocess.check_output(['wc', '-l', groupToProcess + '_hweFiltered.bim'])
			shared_obj[groupToProcess.split('/')[-1]] = [total_snps_analyzed_hwe.split()[0], total_snps_passing_hwe.split()[0]] # store total analyzed and passing for hwe step
			shared_obj[groupToProcess.split('/')[-1]] = shared_obj[groupToProcess.split('/')[-1]] + [groupToProcess + '_hweFiltered.hwe']

			return shared_obj
		
		
		# LD pruning filtering
		def run_LDpruning_VIP(groupToProcess, shared_obj):
			print "running LD pruning step"
			general_plink.run(
				Parameter('--bfile', groupToProcess + '_hweFiltered'),
				Parameter('--'+pipeline_args['LDmethod'], str(pipeline_args['windowSize'])+'kb', str(pipeline_args['stepSize']), str(pipeline_args['VIF'])),
				Parameter('--out', groupToProcess)
			)
							
			total_snps_analyzed_ld = subprocess.check_output(['wc', '-l', groupToProcess + '_hweFiltered.bim'])
			total_snps_passing_ld = subprocess.check_output(['wc', '-l', groupToProcess + '.prune.in'])
			shared_obj[groupToProcess.split('/')[-1]] = [total_snps_analyzed_ld.split()[0]] + [total_snps_passing_ld.split()[0]] # store total analyzed and passing for LD pruning step

			return shared_obj

		def run_LDpruning_rsq(groupToProcess, shared_obj):
			print "running LD pruning step"
			pass;



		# pipeline calls and multiprocessing/parallelization
			...
			if step_order[0] == 'hwe':
				...
				for directories in os.listdir(outdir):
					if ((os.path.isdir(os.path.join(outdir, directories))) and (directories != 'merged_group_files')):
						name=outdir + '/' + directories + '/' + reduced_plink_name + '_' + directories
						#to_be_processed.append(outdir + '/' + directories + '/' + reduced_plink_name + '_' + directories)
						p = multiprocessing.Process(target=run_hwe, args=(name, shared_dict,))
						to_be_processed.append(p)
						p.start()

				for p in to_be_processed:
					p.join() # wait for all processes to finish before continuing

				dict_convert = dict(shared_dict)
				hwe_stats = summary_stats.hwe(dictHWE=dict_convert, thresh=pipeline_args['hweThresh'], outDir = outdir)
