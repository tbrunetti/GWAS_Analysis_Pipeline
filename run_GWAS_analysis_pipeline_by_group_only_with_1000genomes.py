...
class Pipeline(BasePipeline):
	...
	def run_pipeline(self, pipeline_args, pipeline_config):
		...
		step_order = self.check_steps(
			order = ['hwe', 'LD', 'maf', 'het', 'ibd', '1000_genomes', 'KING', 'PCA'], 
			...
			# DUPLICATES MUST BE REMOVED BEFORE USING GENESIS PIPELINE!
				...
				for directories in os.listdir(outdir):
					if (os.path.isdir(os.path.join(outdir, directories))):
						...
						pheno_Genesis = pd.read_table(outdir + '/' + directories + '/' + reduced_plink_name+ '_' + directories +  '_maf_greater_thresh_hetFiltered_dups_removed_thousGen.fam', delim_whitespace=True, names = ['FID,', 'IID', 'PAT', 'MAT', 'SEX', 'AFF'], dtype=str)
			...
			# previous step
				...
				for directories in os.listdir(outdir):
					if (os.path.isdir(os.path.join(outdir, directories))):
						...
						num_snps = sum(1 for line in open(outdir + '/' + directories + '/' + reduced_plink_name+ '_' + directories +  '_maf_greater_thresh_hetFiltered_dups_removed_thousGen.bim'))
						...
						for split_range in range(0, num_files):
							if split_range < 10: # required because linux split uses 00, 01, 02, 03, format therefore when using python iterator a 0 must be appended to front
								total_files.append(outdir + '/' + directories + '/' + reduced_plink_name+ '_' + directories +  '_maf_greater_thresh_hetFiltered_dups_removed_thousGen_split0'+str(split_range)+'.results.txt')
								group_files[directories].append(outdir + '/' + directories + '/' + reduced_plink_name+ '_' + directories +  '_maf_greater_thresh_hetFiltered_dups_removed_thousGen_split0'+str(split_range)+'.results.txt')
							else:
								total_files.append(outdir + '/' + directories + '/' + reduced_plink_name+ '_' + directories +  '_maf_greater_thresh_hetFiltered_dups_removed_thousGen_split'+str(split_range)+'.results.txt')
								group_files[directories].append(outdir + '/' + directories + '/' + reduced_plink_name+ '_' + directories +  '_maf_greater_thresh_hetFiltered_dups_removed_thousGen_split'+str(split_range)+'.results.txt')

						# run a shell script which will submit slurm script
						try:
							check_processes.append(subprocess.Popen(['./export_var_slurm_streamlined_by_group_only.sh',outdir + '/' + directories + '/' + reduced_plink_name+ '_' + directories +  '_maf_greater_thresh_hetFiltered_dups_removed_thousGen',pipeline_config['R_libraries']['path'], pipeline_args['usePCs']]))
				...
				for directories in os.listdir(outdir):
					if (os.path.isdir(os.path.join(outdir, directories))):
						...
						subprocess.call(['head', '-n', '1', outdir + '/' + directories + '/' + reduced_plink_name+ '_' + directories +  '_maf_greater_thresh_hetFiltered_dups_removed_thousGen_split00.results.txt'], stdout=final_results_merged)
						...
						cases_controls = pd.read_table(outdir + '/' + directories + '/' + reduced_plink_name+ '_' + directories +  '_maf_greater_thresh_hetFiltered_dups_removed_thousGen.fam', delim_whitespace=True, names=['FID', 'IID', 'PAT', 'MAT', 'SEX', 'AFF'])	
